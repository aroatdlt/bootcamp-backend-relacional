...
namespace Lemoncode.Azure.Api.Controllers
{
    ...
    public class GamesController : ControllerBase
    {
        ...
        public async Task<IActionResult> DeleteGame(int id)
        {
            ...
            var gameScreenshot = context.Game.Include(i => i.Screenshots).FirstOrDefault(i => i.Id == id);
            if(gameScreenshot == null)
            {
                return NotFound();
               
            }
            var folder = gameScreenshot.Id.ToString();
            await StorageHelper.DeleteFolderBlobs("screenshots", folder, storageOptions);
            context.Game.Remove(gameScreenshot);
